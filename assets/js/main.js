...
(function () {
  ...
  // Menú móvil y accordions responsivos (Drawer deslizable estilo Veris)
  ...
  if (toggle && header && nav) {
    ...
    // Cerrar al elegir una opción (solo si no es un dropdown trigger CON submenú en móvil)
    nav.addEventListener("click", function (e) {
      if (e.target.tagName === "A" && document.body.classList.contains("nav-open")) {
        var isDropdownTrigger = e.target.classList.contains("dropdown-trigger");
        var hasSubmenu = e.target.parentElement && e.target.parentElement.classList.contains("nav-item-dropdown");
        if (window.innerWidth <= 980 && isDropdownTrigger && hasSubmenu) {
          // Es un disparador de acordeón con submenú — no navegar, solo abrir/cerrar submenú
          return;
        }
        // Es un link de navegación normal (incluyendo "Encuéntranos") — cerrar menú y navegar
        closeMenu();
      }
    });

    // Cerrar haciendo click fuera del drawer (en el overlay)
  ...
  // Reubicación de botones CTA y cabecera de cajón en móvil
  function initMobileMenu() {
    ...
    if (mainNav && headerCta) {
      if (window.innerWidth <= 980) {
        ...
        // En desktop, devolverlo a su posición original
        if (mainNav.querySelector(".header-cta")) {
    ...
    // 2. Insertar cabecera de cajón en móvil (Logo + Botón Cerrar)
    if (mainNav && window.innerWidth <= 980) {
      var drawerHeader = mainNav.querySelector(".drawer-header");
      if (!drawerHeader) {
        drawerHeader = document.createElement("div");
        drawerHeader.className = "drawer-header";
        drawerHeader.innerHTML = `
          <img src="assets/img/Iconos/logos/centro%20integrado%20logo.svg" class="drawer-logo" alt="Centro Integrado 360">
          <button class="drawer-close" aria-label="Cerrar menú">&times;</button>
        `;
        mainNav.insertBefore(drawerHeader, mainNav.firstChild);
        
        drawerHeader.querySelector(".drawer-close").addEventListener("click", function(e) {
          e.stopPropagation();
          closeMenu();
        });
      }
    ...
    // 3. Controlar acordeones de submenús en móviles
  ...
  if (homeSlider && sliderTrack && slides.length > 0) {
    ...
    function updateSliderPosition() {
      ...
      if (activeSlide) {
        var wWidth = homeSlider.offsetWidth;
        var sWidth = activeSlide.offsetWidth;
        currentTranslate = -activeSlide.offsetLeft + (wWidth - sWidth) / 2;
    ...
    function updateDots() {
      ...
      // Sincronizar clase active en los slides para efectos de escala y opacidad
